%% plantilla_imagen_C_array
% plantilla_imagen_C_array(imagen,leds,theta,nombre,hueco)
...
function plantilla_imagen_C_array(imagen,leds,theta,nombre,hueco)
...
for angulo = 0:cambio_theta:360-cambio_theta
    ...
    if angulo ~= 360-cambio_theta
        fprintf(fileID,'%d,',lineaR(1+hueco:end));
        fprintf(fileID,'\n');
    else
...
for angulo = 0:cambio_theta:360-cambio_theta
    [lineaR,lineaG,lineaB] = lineaRGB(imagen,leds,angulo);
    if angulo ~= 360-cambio_theta
        fprintf(fileID,'%d,',lineaG(1+hueco:end));
        fprintf(fileID,'\n');
    else
...
for angulo = 0:cambio_theta:360-cambio_theta
    [lineaR,lineaG,lineaB] = lineaRGB(imagen,leds,angulo);
    if angulo ~= 360-cambio_theta
        fprintf(fileID,'%d,',lineaB(1+hueco:end));
        fprintf(fileID,'\n');
    else

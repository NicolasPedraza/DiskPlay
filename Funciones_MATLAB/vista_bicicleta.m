%% vista_bicicleta
% [IB,IC,IO] = vista_bicicleta(nombre,leds)
...
% IB: Imagen bicicleta
function [IB,IC,IO] = vista_bicicleta(nombre,leds)
I = imread(nombre);
IO = I;
...
if x>y
    ...
    IB2(:,1:size(complemento,2),:)=[];
    IB2(:,end-size(complemento,2)+1:end,:)=[];
end
if x<y
    IC(1:size(complemento,1),:,:)=[];
    IC(end-size(complemento,1)+1:end,:,:)=[];
    IB2(1:size(complemento,1),:,:)=[];
    IB2(end-size(complemento,1)+1:end,:,:)=[];
end
%Cambiar tamaño (Vistas)
IB = imresize(IB2, [leds*2+1 leds*2+1]);
